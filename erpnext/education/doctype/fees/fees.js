...
frappe.ui.form.on("Fees", {
	...
	program_enrollment: function (frm) {
		if (frm.doc.program_enrollment) {
			frappe.call({
				method: "erpnext.education.api.get_p_details",
				args: { user: frm.doc.program_enrollment },
				callback: function (r) {
					if (r.message) {
						frm.set_value("program", r.message[0]);
						frm.set_value("academic_year", r.message[1]);
						frm.set_value("academic_term", r.message[2]);
					}
				},
			});
			frappe.call({
			...
			callback: (data) => {
				...
				if (frm.doc.no_of_siblings == 1) {
					for (var i = 0; i < frm.doc.siblings.length; i++) {
						if (
							s_date_of_birth > frm.doc.siblings[i].date_of_birth
						) {
					...
					if (
						s_date_of_birth > sibling1 &&
						s_date_of_birth > sibling2
					) {
						...
						frappe.msgprint(
							"He is Not Eligible for Sibling discount"
						);
					}
